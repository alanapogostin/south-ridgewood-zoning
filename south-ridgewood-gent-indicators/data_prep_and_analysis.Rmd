...
age_by_sex <- get_acs(geography = "tract", 
  ...
  geometry = FALSE,
                      variables = c("m_0_4" = "B01001_003",
                                    ...
                                    "f_85p" = "B01001_049"), 
                      year = 2019,
                      state = "NY", county = "081",survey = "acs5") %>% 
  filter(GEOID %in% c(36081055500,
                    36081055300,
                    36081055700,
...
age_by_sex_clean  <- age_by_sex %>%
  ...
  mutate(neighborhood_total = `36047005300` + `36047005900` + `36047008500`) %>% 
...
pop_pyramid <- ggplot(age_by_sex_dataframe, aes(x = age_group, y = value, fill = Gender)) + 
  ...
  scale_y_continuous(limits = c(-0.1,0.1), breaks = seq(-0.1, 0.1, 0.02), labels = abs(seq(-0.1, 0.1, 0.02)))+
  # set the x and y axis displayed names
  xlab("Age Group") +
  ylab("Percentage") +
  # set the title text
  ggtitle("Population Pyramid - Red Hook, ACS 2014-2018")+
  # turn the graph 90 degrees to make it into a population pyramid
  coord_flip()
...
gg_pop_change <- ggplot(pop_change, aes(x = year, y = pop)) +
  geom_bar(stat="identity") +
  scale_y_continuous(n.breaks = 10) +
  scale_x_continuous(label = c(0, 2009, 2014, 2019))+
  #scale_x_continuous(labels = as.character(year))+
